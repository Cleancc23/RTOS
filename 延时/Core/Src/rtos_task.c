...
{	
 TaskHandle_t tmp_handle;	  //一次性任务
	
 TaskHandle_t xHandle;		//其他主任务
 ...
 xTaskCreate(two_led_toggle,"two_led_toggle_task",128,(void *)0,11,&xHandle);	  //优先级要小于一次性任务的15
	
 printf("create 2nd task\r\n");

 xTaskCreate(print_string,"print_string_task",128,(void *)0,10,&xHandle2);	//优先级要小于一次性任务的15
	 	
 tmp_handle = xTaskGetHandle("runonce_entry_task");
